...
/**
 * Superadmin SLA Repository
 * 
 * Handles data access for system SLA monitoring (incidents, uptime, patches).
 * Provides superadmins with comprehensive system health and SLA metrics.
 */
class SuperadminSLARepository implements SuperadminSLARepositoryInterface
{
    /**
    ...
    {
        $logs = ApplicationUptimeLog::inDateRange($startDate, $endDate)->get();
     ...
     */
    public function getLatestUptimeLog(): ?ApplicationUptimeLog
    {
        return ApplicationUptimeLog::latest('checked_at')->first();
    }
    ...
    {
        $lastDowntime = ApplicationUptimeLog::downtime()
            ->latest('checked_at')
            ->first();

        if (!$lastDowntime) {
            // No downtime recorded, check first log entry
            $firstLog = ApplicationUptimeLog::oldest('checked_at')->first();
    ...
    {
        return ApplicationUptimeLog::downtime()
            ->latest('checked_at')
            ->limit($limit)
            ->get();
    }
