...
export default function SuperadminDashboard({ counts, company, onboardingStatus, showSetupModal = false, welcomeText = '' }: { counts?: { users?: number }; company?: { name?: string }; onboardingStatus?: string; showSetupModal?: boolean; welcomeText?: string }) {
                    ...
                    <div className="rounded-lg border p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <div className="font-medium">{company?.name ?? 'Not configured'}</div>
                            </div>
                            <div>
                                <span className={`inline-flex items-center rounded px-2 py-1 text-xs ${onboardingStatus === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                    {onboardingStatus === 'completed' ? 'Configured' : onboardingStatus === 'skipped' ? 'Skipped' : 'Not configured'}
                                </span>
                            </div>
                        </div>
                    </div>
